...
%   - epsr: relative permittivity of 
%   - mur: relative permeability of 
...
function effPars=txLineParsfromS(Smat,freqsr,epsr,mur,l,Zref,varargin)
    c=3e8;
    dl=varargin{1};         %de-embedding length    
    epsr_pt=varargin{2};    %port material
    ...
    S21=S21.*exp(-1i*2*2*pi*freqsr/c*sqrt(mur_pt*epsr_pt)*dl);
